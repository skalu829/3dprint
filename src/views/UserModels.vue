        ...
        >
          ...
          <div class="card-footer">
            <div class="card-stats">
              <span class="stat">⬇ {{ formatNum(m.downloads) }}</span>
              <span class="stat">❤ {{ formatNum(m.likes) }}</span>
            </div>
            <span class="card-date">{{ formatDate(m.createdAt) }}</span>
          </div>
        </div>
      </div>
    </main>
...
async function fetchMyModels() {
  ...
  try {
    ...
    models.value = (data || []).filter(m => m.userId === user.id)
  } catch (err) {
